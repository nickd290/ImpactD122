...
// Mark/unmark job as duplicate
export const toggleDuplicateFlag = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const job = await prisma.job.update({
      where: { id },
      data: { isDuplicate: isDuplicate !== undefined ? isDuplicate : true },
      include: {
        customer: true,
        vendor: true,
        lineItems: true,
    ...
    console.error('Toggle duplicate flag error:', error);
    res.status(500).json({ error: 'Failed to update duplicate flag' });
  }
};
...
export const detectDuplicates = async (req: Request, res: Response) => {
  try {
    // Find jobs with same title and customer within 7 days
    ...
    const potentialDuplicates = Object.values(duplicateGroups)
